...
func (i *Inbound) NewConnectionEx(ctx context.Context, conn gonet.Conn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	...
	sourceDest, err := singbridge.ToDestination(source, net.Network_TCP)
	if err != nil {
		errors.LogWarning(ctx, "invalid tuic source: ", err)
		return
	}
	...
	if destination.IsValid() {
		targetDest, err = singbridge.ToDestination(destination, net.Network_TCP)
		if err != nil {
			errors.LogWarning(sessionCtx, "invalid tuic destination: ", err)
			return
		}
	} else {
...
func (i *Inbound) NewPacketConnectionEx(ctx context.Context, conn N.PacketConn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	...
	sourceDest, err := singbridge.ToDestination(source, net.Network_UDP)
	if err != nil {
		errors.LogWarning(ctx, "invalid tuic UDP source: ", err)
		return
	}
	// Build session inbound
	inbound := &session.Inbound{
	...
	var targetDest net.Destination
	if destination.IsValid() {
		targetDest, err = singbridge.ToDestination(destination, net.Network_UDP)
		if err != nil {
			errors.LogWarning(sessionCtx, "invalid tuic UDP destination: ", err)
			return
		}
	} else {
