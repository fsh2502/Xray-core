...
func (o *Outbound) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	// The SYN frame has no body; the first PSH carries the destination.
	synBody := []byte{}
	if len(outbounds) > 0 {
		ob := outbounds[len(outbounds)-1]
		if ob.Target.IsValid() {
	...
	writeFrameBuffered(cmdSYN, mySid, nil)
	writeFrameBuffered(cmdPSH, mySid, synBody)

	// Register before flushing so a fast server response cannot be lost.
				...
				if scheme != nil && uint32(packetIndex) < scheme.stop {
					...
					if len(pktSizes) > 0 {
						for _, size := range pktSizes {
							...
							// Padding is a waste frame, never application payload.
							if werr := sendFrame(cmdWaste, 0, make([]byte, size-remaining)); werr != nil {
								buf.ReleaseMulti(mb)
								return
							}
						}
